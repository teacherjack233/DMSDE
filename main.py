...
from training.trainer import (
    clear_training_complete_marker,
...
def main():
    ...
    if checkpoint_path is None:
        components = []
        component = None
        clear_training_complete_marker(args.model_dir)
        print(
    ...
    mark_training_started(args.model_dir)
    ...
    for stream_id, stream_dataset in enumerate(
        ...
        for batch_idx, (data, labels) in enumerate(task_loader):
            ...
            distance = memory.compute_vae_mmd(
                component,
                batch_size=args.encode_batch_size,
            )
            should_expand = distance > args.threshold

            distance_label = f"{component.vae_arch.upper()}-MMD"
            writer.add_scalar(f"SDM/{component.vae_arch}_mmd", distance, decision_step)
            ...
            print(
                ...
                f"{distance_label}={distance:.6f}, threshold={args.threshold:.6f}, "
                f"expand={should_expand}, experts={len(components)}"
            )
            logging.info(
                "SDM decision %s | %s=%.6f threshold=%.6f expand=%s experts=%s",
                decision_step,
                distance_label,
                distance,
                args.threshold,
            ...
            if should_expand:
                ...
                if args.sample_selection:
                    consolidation_msg = (
                        ...
                        f"into long memory by {component.vae_arch.upper()} ELBO, then cleared short memory"
                    )
                else:
                    consolidation_msg = (
                        f"No expansion: appended {selected_from_short} short-memory samples "
                        "into long memory by FIFO sliding window, then cleared short memory"
                    )
                print(consolidation_msg)
                logging.info(
                    "No expansion | sample_selection=%s selected_from_short=%s",
    ...
    if components:
        ...
        mark_training_complete(args.model_dir)
