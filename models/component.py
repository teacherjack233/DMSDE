...
class Component:
    """One DMSDE expert: a spiking VAE plus a classifier."""
    ...
    def __init__(
        ...
        self.vae_arch = vae_arch.lower()
        self.vae_mmd_type = vae_mmd_type
        self.vae_distance_lambda = vae_distance_lambda
        self.feature_mode = feature_mode
        self.device = torch.device("cuda" if torch.cuda.is_available() else "cpu")
        self.is_frozen = False

        self.vae = self._build_vae().to(self.device)
    ...
    def _build_vae(self):
        if self.feature_mode:
            raise ValueError("Unsupported feature mode for the public configuration.")
        if self.vae_arch == "fsvae":
            return SpikingFSVAE(in_channels=self.input_channels, n_steps=self.n_steps)
        raise ValueError(f"Unsupported vae_arch: {self.vae_arch}")
    ...
    def compute_information_scores(self, x, batch_size=None):
        """Negative-ELBO proxy used by DMSDE memory consolidation."""
    ...
    def encode_mmd_features(self, x, batch_size=None):
        ...
        with torch.no_grad():
            for start in range(0, x.size(0), batch_size):
                ...
                if hasattr(self.vae, "encode_mmd_features"):
                    encoded = self.vae.encode_mmd_features(spike_input)
                else:
                    _, encoded, _ = self.vae.encode(spike_input)
