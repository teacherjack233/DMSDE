...
class DualMemoryBuffer:
    ...
    def compute_vae_mmd(self, component, batch_size=64):
        short_samples, _ = self.get_short_samples(shuffle=False)
    ...
    def compute_fsvae_mmd(self, component, batch_size=64):
        return self.compute_vae_mmd(component, batch_size=batch_size)

    def consolidate(self, component, batch_size=64, sample_selection=True):
        if not self.short_buffer:
            return 0

        if not sample_selection:
            accepted_from_short = len(self.short_buffer)
            self.long_buffer.extend(self.short_buffer)
            overflow = len(self.long_buffer) - self.long_size
            if overflow > 0:
                self.long_buffer = self.long_buffer[overflow:]
            return accepted_from_short
    ...
    def print_statistics(self):
        ...
        if stats["last_distance"] is not None:
            print(f"Last VAE-MMD distance: {stats['last_distance']:.6f}")
        print("======================================")
